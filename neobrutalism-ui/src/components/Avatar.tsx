...
const avatarVariants = cva(
  'inline-flex items-center justify-center font-bold border-3 border-black shadow-brutal-sm bg-white overflow-hidden',
  {
    variants: {
      size: {
        xs: 'h-6 w-6 text-xs',
...
>(({ className, max = 5, children, ...props }, ref) => {
  ...
  return (
    ...
    >
      {visibleChildren}
      {extraCount > 0 && (
        <div className="inline-flex items-center justify-center h-10 w-10 border-3 border-black bg-gray-200 text-sm font-bold shadow-brutal-sm">
          +{extraCount}
        </div>
      )}
    </div>
