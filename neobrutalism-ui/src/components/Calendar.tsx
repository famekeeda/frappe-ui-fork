  ...
  (
    ...
    const renderCalendar = () => {
      ...
      for (let i = startDay - 1; i >= 0; i--) {
        days.push(
          <div
            key={`prev-${i}`}
            className="p-2 text-gray-400 text-center text-sm font-medium"
          >
            {prevMonthDays - i}
          </div>
        )
          ...
          <div
            ...
            className={cn(
              ...
              disabled
                ...
                : 'border-transparent',
              isToday && 'border-black font-black',
              isSelected && 'bg-black text-white',
              (isRangeStart || isRangeEnd) && 'bg-black text-white',
              inRange && !isRangeStart && !isRangeEnd && 'bg-gray-200',
                ...
                {dayEvents.slice(0, 3).map((event) => (
                  <div
                    ...
                    className={cn(
                      ...
                      (!event.color || event.color === 'black') && 'bg-black'
                    )}
                    title={event.title}
                  />
                ))}
                {dayEvents.length > 3 && (
                  <div className="h-1.5 w-1.5 rounded-full bg-gray-400" title={`+${dayEvents.length - 3} more`} />
                )}
              </div>
            )}
          </div>
    ...
    return (
      <div
        ref={ref}
        className={cn(
          'border-3 border-black bg-white shadow-brutal p-4',
          className
        )}
        {...props}
      >
        ...
        <div className="flex items-center justify-between mb-4">
          <button
            ...
            className="p-2 border-2 border-black bg-white hover:bg-gray-100 transition-all hover:translate-x-0.5 hover:translate-y-0.5"
          >
        ...
        <div className="grid grid-cols-7 gap-1 mb-2">
          {showWeekNumbers && <div className="p-2 text-center text-xs font-black text-gray-500">W</div>}
          {dayNames.map((day) => (
            <div key={day} className="p-2 text-center text-xs font-black text-gray-500">
              {day}
            </div>
          ))}
        </div>
